    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            CBUFFER_START(UnityPerMaterial)
                ...
                half _Emission, _Usecenterglow, _Usedepth, _Usecustomrandom, _Depthpower, _CullMode, _Blend2;
            CBUFFER_END
            ...
            half4 frag(Varyings input) : SV_Target
            {
                ...
                half4 noise = SAMPLE_TEXTURE2D(_Noise, sampler_Noise, noiseUv + input.uv.zw * _Usecustomrandom);
                half centerGlow = lerp(1.0h, saturate(1.0h - length(input.uv.xy - 0.5h) * 2.0h), saturate(_Usecenterglow));
                half alpha = mainTex.a * noise.a * mask.a * input.color.a * _Color.a * centerGlow;
                half3 rgb = mainTex.rgb * noise.rgb * _Color.rgb * input.color.rgb * _Emission * alpha;
